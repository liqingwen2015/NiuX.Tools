...
namespace NiuX.DependencyInjection
{
    ...
    /// 服务容器
    /// </summary>
    internal sealed partial class ServiceContainer : IServiceContainer
    {
        ...
        public IServiceContainer CreateScope() => new ServiceContainer(this);
        ...
        private object? EnrichObject(object? obj)
        {
            ...
            // PropertyInjection
            foreach (var property in TypeCacheHelper.GetTypeProperties(type).Where(x => x.IsDefined(typeof(FromServiceAttribute))))
            {
        ...
        private object? GetServiceInstanceInternal(Type serviceType, ServiceDefinition serviceDefinition)
        {
            ...
            var newFunc = TypeCacheHelper.TypeNewFuncCache.GetOrAdd(implementType, (serviceContainer) =>
            {
                if (TypeCacheHelper.TypeEmptyConstructorFuncCache.TryGetValue(implementType, out var emptyFunc))
                {
                    return emptyFunc.Invoke();
                }

                var ctor = TypeCacheHelper.TypeConstructorCache.GetOrAdd(implementType, t =>
                {
                ...
                if (parameters.Length == 0)
                {
                    ...
                    TypeCacheHelper.TypeEmptyConstructorFuncCache.TryAdd(implementType, func00);
                ...
                var func = TypeCacheHelper.TypeConstructorFuncCache.GetOrAdd(implementType, t =>
                {
                    if (!TypeCacheHelper.TypeConstructorCache.TryGetValue(t, out var ctorInfo) || ctorInfo is null)
                    {
                        return null!;
                    }
